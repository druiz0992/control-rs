...
use crate::numeric_services::differentiation::{DerivativeRequest, DerivativeResponse};
use crate::numeric_services::traits::DerivativeEngine;

const PYTHON_SCRIPT_PATH: &str = "src/numerical_methods/autodiff/backend.py";
...
impl DerivativeEngine for Sympy {
    fn compute_derivatives(
    ...
    ) -> Result<DerivativeResponse, &'static str> {
        // Set up the Python process
        let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR")); // project root
        if !path.exists() {
            return Err("Python backend script not found at expected path");
        }
        ...
        let mut child = Command::new(PYTHON_INTERPRETER)
            ...
            .stdout(Stdio::piped())
            .spawn()
            .map_err(|_| "Failed to start Python process")?;
        ...
        if let Some(stdin) = child.stdin.as_mut() {
            stdin
                ...
                .map_err(|_| "Failed to send input data to Python")?;
        }

        // Capture the output (the gradient and Hessian)
        let output = child
            .wait_with_output()
            .map_err(|_| "Failed to read Python output")?;
        ...
        */
        ...
        // convert python specific notation to rust's
