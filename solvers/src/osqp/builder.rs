...
impl<'a> OSQPBuilder<'a> {
    ...
    pub fn build(self) -> Result<(OSQPSolverHandle, QPParams<'a>), SolverError> {
        self.check()?;
        ...
        let solver = OSQPSolver {
            problem: Problem::new(q_csc, &q, a_csc, &l, &u, options)
                .map_err(|e| SolverError::ConfigError(e.to_string()))?,
            n_eq,
            n_ineq,
        };
    ...
    fn check(&self) -> Result<(), SolverError> {
        ...
        if self.qp_params.q_mat.is_none() && self.qp_params.q_csc.is_none() {
            return Err(SolverError::ConfigError("q_mat is required".into()));
        }

        if self.qp_params.q_vec.is_none() {
            return Err(SolverError::ConfigError("q_vec is required".into()));
        }
