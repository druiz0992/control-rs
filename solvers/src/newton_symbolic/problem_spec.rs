...
use core::fmt;
use symbolic_services::symbolic::{ExprScalar, ExprVector, SymbolicFn, SymbolicFunction};
...
impl ProblemSpec {
    ...
    pub fn get_params(
        &self,
    ) -> Result<(&SymbolicFunction, &SymbolicFunction, Vec<ExprScalar>), SolverError> {
        let residual_fn = self
            .residual
            .as_ref()
            .ok_or_else(|| SolverError::ConfigError("Residual not configured".to_string()))?;
        let jacobian_fn = self
            .jacobian
            .as_ref()
            .ok_or_else(|| SolverError::ConfigError("Jacobian not configured".to_string()))?;
        let unknown_vars = self.unknown_vars.as_ref().ok_or_else(|| {
            SolverError::ConfigError("Unknown variables not configured".to_string())
        })?;
    ...
    > {
        ...
        let ip_residual_fn = self.ip_residual.as_ref().ok_or_else(|| {
            SolverError::ConfigError("Interior Point Residual not configured".to_string())
        })?;
        ...
        let unknown_vars = self.unknown_vars.as_ref().ok_or_else(|| {
            SolverError::ConfigError(
            "Unknown variables not configured".to_string())
        })?;
