...
impl DerivativeEngine for Sympy {
    fn compute_derivatives(
        ...
        let status = child
            ...
            .map_err(|_| DerivativeError::Other("Failed to wait with timeout".to_string()))?;

        let output = match status {
            Some(status) if status.success() => {
                child
                    .wait_with_output()
                    .map_err(|_| DerivativeError::Other("Failed to read output".to_string()))?
                // Proceed with output
            }
            Some(status) => {
                return Err(DerivativeError::Other(format!(
                    "Process exited: {:?}",
                    status
                )));
            }
            None => {
                child.kill().ok(); // kill the process
                return Err(DerivativeError::Other(
                    "Python process timed out".to_string(),
                ));
            }
        };
