...
impl<const N: usize, const C: usize> Dynamics for LtiModel<N, C> {
    type State = LtiState<N, C>;

    fn dynamics(&self, state: &Self::State, input: Option<&[f64]>) -> Self::State {
        let u = DVector::from_row_slice(input.unwrap_or(&vec![0.0; self.get_b_as_slice().ncols()]));
        let s =
...
impl<const N: usize, const C: usize> LinearDynamics for LtiModel<N, C> {
