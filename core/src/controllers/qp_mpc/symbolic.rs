...
use crate::controllers::{
    Controller, ControllerState, CostFn, SteppableController, TrajectoryHistory,
    UpdatableController,
};
...
pub struct ConvexMpcSymbolic<S: PhysicsSim, C: SteppableController<S> + UpdatableController<S>>(
    ConvexMpcGeneric<S, C>,
);

impl<S> ConvexMpcSymbolic<S, QPLQRSymbolic<S>>
where
    S: PhysicsSim,
...
impl<S> Controller<S> for ConvexMpcSymbolic<S, QPLQRSymbolic<S>>
where
    S: PhysicsSim,
    S::Model: SymbolicDynamics + Labelizable,
    S::Discretizer: SymbolicDiscretizer<S::Model>,
